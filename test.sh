...
if ps -p $SERVER2_PID > /dev/null; then
    ...
    cat tests/server2.log
    exit 1
fi

# Check if the server log contains any errors (e.g., "ERROR" keyword)
if grep -i "error" tests/server2.log; then
    echo "Error found in server2 log."
    cat tests/server2.log
    kill $SERVER2_PID
    exit 1
fi
...
./testClient2 > tests/client2.log 2>&1 &
CLIENT2_PID=$!
wait $CLIENT2_PID
...
sed -E 's/\[[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}\]//g' tests/client2.log | \
...
sed -E 's/\[[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}\]//g' tests/server2.log | \
sed 's/"client-id":"[^"]*"/"client-id":"<client-id>"/g' | \
sed 's/"server-id":"[^"]*"/"server-id":"<server-id>"/g' | \
